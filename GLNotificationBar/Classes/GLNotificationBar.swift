...
extension UITableViewCell{
    func drawSeparatorLine() {
        ...
        border.frame = CGRect(x: 0, y: 49,
                              width: 0, height: 2)
        let height:CGFloat = (APP_DELEGATE.keyWindow?.frame.size.height)!
        let width:CGFloat = (APP_DELEGATE.keyWindow?.frame.size.width)!
        
        border.frame.size.width = (height > width ? APP_DELEGATE.keyWindow?.frame.size.height : APP_DELEGATE.keyWindow?.frame.size.width)!
